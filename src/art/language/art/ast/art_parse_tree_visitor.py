...
class ArtParseTreeVisitor(ParseTreeVisitor):
    """
    """
    def __init__(self, cst, grammar):
        ...
        self.tree = ParseTreeFactory.make_tree(ArtParseTreeKind.UNKNOWN, self.grammar)
    ...
    def make_tree(self, tree, papa=None):
        """
        """
        result = None
        if self.accepted_non_terminal(tree.kind) or self.accepted_terminal(tree.kind):
            result = ParseTreeFactory.clone(tree)
        if papa and result:
            papa.add_kid(result)
        return result
    ...
    def visit(self, tree, *args, **kwargs):
        ...
        """
        stack = deque()
        self.tree = papa = self.make_tree(tree)  # set root, avoid update self.tree in the loop
        for kid in reversed(tree.kids):
            if self.accepted_non_terminal(kid.kind) or self.accepted_terminal(kid.kind):
                stack.append(ParseTreeVisitor.TvpPair(kid, 1, papa))
        while stack:
            pair = stack.pop()
            if pair.value == 1:
                pair.value += 1
                ...
                assert papa, 'Unable to build a new tree.'
